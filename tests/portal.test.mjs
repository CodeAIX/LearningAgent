...
test("medical container serves the portal independently of the platform", async (t) => {
  ...
  for (const path of ["/", "/admin", "/admin/"]) {
    ...
    assert.doesNotMatch(page.body, /class="constellation"/);
  }
  for (const asset of ["/covers/laboratory.webp", "/favicon.svg"])
    assert.equal((await app.inject(asset)).statusCode, 200);
  for (const path of [
    "/platform/",
    "/platform/home.js",
    "/platform/home.css",
