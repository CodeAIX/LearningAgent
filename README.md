...
v1.2.0 起，医学站与总平台分别构建、部署、升级和回退：

| 项目 | 域名 | VPS 本地端口 |
| --- | --- | --- |
| 本项目 LearningAgent | `med.aixico.com` | `127.0.0.1:18082` |
| [AixicoHome 总平台](https://github.com/CodeAIX/AixicoHome) | `aixico.com` | `127.0.0.1:18083` |

两站仍复用宿主机的 Cloudflare Tunnel。总平台为无数据库的静态 Nginx 容器；医学站保留现有后台、SQLite、上传图片与备份。

从曾经合并的 v1.1.0 升级时，应先安装 AixicoHome 并确认健康，再将根域名 Tunnel 改到 `18083`，最后升级本项目。医学站的 `SITE_ORIGIN`、端口、数据库和账号保持原样。
...
以 v1.2.0 为例，一条命令下载固定版本引导脚本并开始安装：

```bash
curl -fL --retry 3 https://github.com/CodeAIX/LearningAgent/releases/download/v1.2.0/bootstrap.sh -o /tmp/learning-portal-bootstrap.sh && sudo bash /tmp/learning-portal-bootstrap.sh v1.2.0 https://med.aixico.com 18082
```
...
sudo learning-portal upgrade v1.2.0
...
- 总平台由独立 AixicoHome 项目维护和备份，不包含在本项目镜像或数据备份内。
...
```bash
docker buildx build --platform linux/amd64 --load -f infra/Dockerfile --build-arg REVISION="$(git rev-parse HEAD)" -t ghcr.io/codeaix/learning-portal:v1.2.0 .
```
